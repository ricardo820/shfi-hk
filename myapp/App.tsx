      ...
      await Promise.all(
        receivableTransfers.map((transfer) =>
          publishDebtPushedNotification(openedRoom.id, {
            fromUserIds: [Number(transfer.debtorId)],
            toUserId: authenticatedUser.id,
            amount: Number(transfer.amount.toFixed(2)),
            currency: 'EUR',
            note: 'Debt reminder generated by creditor',
          })
        )
      );

      setRoomDetailsStatus('Debt notifications sent to debtors who owe you.');
    } catch (error) {
      if (axios.isAxiosError(error)) {
        const serverMessage =
          typeof error.response?.data?.message === 'string'
        ...
        if (error.response?.status === 403) {
          setRoomDetailsError(
            serverMessage || 'Server rejected debt notification request.'
          );
          return;
        }
